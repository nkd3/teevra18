...
def to_frame(payload: dict, meta: dict) -> pd.DataFrame:
    """
    Build a normalized DataFrame from Dhan payload.
    Works for both daily and intraday. Safe when payload arrays are empty.
    """
    ...
    oi_arr = payload.get("open_interest", []) or []
    ts_arr = payload.get("timestamp", []) or []

    # If OI missing or length mismatch, pad with zeros to len(opens)
    if not oi_arr or len(oi_arr) != len(opens):
        oi_arr = [0] * len(opens)
    ...
    df = pd.DataFrame({
        ...
        "open_interest": pd.to_numeric(oi_arr, errors="coerce"),
    })

    # Timestamps (seconds since epoch per Dhan docs)
    if ts_arr:
        ts_utc = pd.to_datetime(ts_arr, unit="s", utc=True)
    else:
        # empty series with timezone
        ts_utc = pd.to_datetime([], utc=True)

    df["ts_utc"] = ts_utc
    # IST = Asia/Kolkata
    df["ts_ist"] = df["ts_utc"].dt.tz_convert("Asia/Kolkata") if len(df) else pd.to_datetime([], utc=True).tz_convert("Asia/Kolkata")

    # Meta
    df["securityId"] = str(meta.get("securityId", ""))
    df["symbol"]     = meta.get("symbol", None)
    df["segment"]    = meta.get("segment", "")
    df["instrument"] = meta.get("instrument", "")
    df["expiryCode"] = int(meta.get("expiryCode", 0))
    df["timeframe"]  = meta.get("timeframe", "")

    # Partition cols — ALWAYS create first, then cast
    df["year"]  = df["ts_utc"].dt.year.astype("Int16")  # nullable Int for safety on empty
    df["month"] = df["ts_utc"].dt.month.astype("Int8")

    # If you prefer plain numpy dtypes (no pandas NA), cast only when non-empty:
    if len(df):
