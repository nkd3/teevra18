...
from t18_common.admin_utils import get_alerts_map, upsert_alert_setting
...
with get_conn() as conn:
    # quick Telegram sanity
    tele = get_alerts_map(conn, "telegram")
    ok = bool(tele.get("BOT_TOKEN") and tele.get("CHAT_ID") and tele.get("ENABLED","0")=="1")
    if ok:
    ...
    with c1:
        if st.button("Start Live Session (Manual Zerodha)"):
            ...
            conn.execute("""INSERT INTO runs (kind,status,lab_id,params_json,started_by,notes)
    ...
    with c2:
        if st.button("Stop Live Session"):
            conn.execute("""UPDATE runs SET status='stopped', ended_at=datetime('now')
    ...
    df_runs = read_df(conn, "SELECT id, kind, status, started_by, started_at, ended_at, notes FROM runs WHERE kind='live' ORDER BY id DESC LIMIT 50")
