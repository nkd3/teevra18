...
with get_conn() as conn:
    ...
    lab_id = st.selectbox("Lab Config", options=df_lab["id"].tolist(), format_func=lambda x: f"{x} — {df_lab[df_lab['id']==x].iloc[0]['name']}") if not df_lab.empty else None

    # status panel
    st.subheader("Session Control")
    c1, c2 = st.columns(2)
    with c1:
        if st.button("Start Paper Session"):
            ...
            conn.execute("""INSERT INTO runs (kind,status,lab_id,params_json,started_by,notes)
                            ...
                            (int(lab_id), json.dumps(params), user.get("username","user")))
            conn.commit()
